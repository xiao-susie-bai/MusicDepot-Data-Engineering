...
DATA_BUCKET_NAME = "de-c4w4a2-533267227814-us-east-1-data-lake"
SCRIPTS_BUCKET_NAME = "de-c4w4a2-533267227814-us-east-1-scripts"
...
def deftunes_songs_pipeline():
    ...
    rds_extract_glue_job = GlueJobOperator(
        ...
        # The name of the Glue job we created in Terraform. This corresponds to the value that we can find in the Terraform output with the key `glue_rds_extract_job`
        job_name="de-c4w4a2-rds-extract-job",
        # Location in the S3 bucket that we used to deploy our scripts
        script_location=f"s3://{SCRIPTS_BUCKET_NAME}/de-c4w4a2-extract-songs-job.py",
        job_desc="Glue Job to extract data from RDS",
        # (*) The name of the IAM role that we deployed with Terraform and can be found in the Terraform output `glue_role_arn`.
        # (*) Take into account that we only need the name instead of the complete ARN
        iam_role_name="de-c4w4a2-glue-role",
        s3_bucket=f"{SCRIPTS_BUCKET_NAME}",
        region_name="us-east-1",
        # Configuration settings for Glue, we will state the Glue version and the number of workers
        create_job_kwargs={
            "GlueVersion": "3.0",
            "NumberOfWorkers": 2,
            "WorkerType": "G.1X",
        ...
        # The parameters depend on the job but we can find the data lake bucket name, the RDS connection, and the ingestion date. 
        # For the ingestion date, pay attention to the usage of Jinja templating to get the next date. 
        # Currently the `"{{ next_ds }}"` variable is deprecated but we are using it by simplicity to take the next start date.
        script_args={
    ...
    dq_check_songs_job = GlueDataQualityRuleSetEvaluationRunOperator(
        ...
        # (*) In this case, instead of just the name, we are required to pass the complete ARN as in the Terraform output `glue_role_arn`.
        role="arn:aws:iam::533267227814:role/de-c4w4a2-glue-role",
        # A list with the set of rules that we want to evaluate. For the `songs` data coming from the RDS, the name of the rule set is in the Terraform output `songs_db_ruleset_name`.
    ...
    # Once the quality checks are evaluated, we will use the `DockerOperator` in the task named `task_dbt`. 
