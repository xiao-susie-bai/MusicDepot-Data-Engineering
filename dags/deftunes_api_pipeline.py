...
API_URL = "ec2-34-199-185-52.compute-1.amazonaws.com"
...
def deftunes_pipeline():
    ...
    # This task will evaluate the rule set for users that we created with Terraform and associated with the Terraform output `users_db_ruleset_name`. 
    # We will use the same Glue Database and the `"users"` table.
    ...
    # We will use the `DockerOperator` in the task named `task_dbt` to use DBT.
